...
def test_language_nonsense_but_translation_ok():
    with patch("src.translator.get_language", return_value="???"), \
         patch("src.translator.get_translation", return_value="Hello there"):
...
def test_classifier_raises_then_fallback():
    with patch("src.translator.get_language", side_effect=RuntimeError("down")), \
...
def test_translation_raises_exception():
    with patch("src.translator.get_language", return_value="German"), \
         patch("src.translator.get_translation", side_effect=RuntimeError("boom")):
        out = translate_content("Hier ist dein erstes Beispiel.")
        assert out == (False, "Unable to translate")
...
def test_unintelligible_input_short_noise():
    with patch("src.translator.get_language", return_value="Spanish"):
